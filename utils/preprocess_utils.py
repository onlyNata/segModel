# -*- coding: utf-8 -*-
"""
Created on Thu Jun 14 11:18:34 2018
...
def pad_to_bounding_box(image, offset_height, offset_width, target_height,
    ...
    target_width_assert = tf.Assert(
                tf.greater_equal(
                    target_width, width),['target_width must be >= width'])
...
#            [], maxval=max_offset_width, dtype=tf.int32)
    ...
    offset_height=tf.div(max_offset_height,2)
    offset_width=tf.div(max_offset_width,2)
...
#%%
#TODO lijihw centerCorp    
def center_crop(image_list, crop_height, crop_width):
    """Crops the given list of images.
...
def resize_image(image,
  ...
  with tf.name_scope(scope, 'resize_to_range', [image]):
    ...
    new_size = tf.stack([height_size, width_size])
